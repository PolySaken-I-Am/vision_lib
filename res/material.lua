...
function visionLib.Material.CreateVariants (node, recipes)
	assert(minetest.registered_nodes[node],'Attempt to create variants for nonexistant node')
	local def = visionLib.Table.Clone(minetest.registered_nodes[node])
	
	minetest.register_node(':'..node..'_bricks', visionLib.Table.Merge(def, {description=def.description..' Bricks',tiles={def.tiles[1]..'^visionlib_bricks_overlay.png'}}))
	
	minetest.register_node(':'..node..'_tiles', visionLib.Table.Merge(def, {description=def.description..' Tiles',tiles={def.tiles[1]..'^visionlib_tiles_overlay.png'}}))
	
	minetest.register_node(':'..node..'_bevelled', visionLib.Table.Merge(def, {description='Bevelled '..def.description,tiles={def.tiles[1]..'^visionlib_border_overlay.png'}}))
	
	minetest.register_node(':'..node..'_pillar', visionLib.Table.Merge(def, {description=def.description..' Pillar',tiles={def.tiles[1]..'^visionlib_circle_carving_overlay.png', def.tiles[1]..'^visionlib_circle_carving_overlay.png', def.tiles[1]..'^visionlib_pillar_overlay.png'}, paramtype2 = 'facedir',	on_place = minetest.rotate_node}))
	
	minetest.register_node(':'..node..'_pillar_2', visionLib.Table.Merge(def, {description='Small '..def.description..' Pillar',tiles={def.tiles[1]..'^visionlib_circle_carving_overlay.png', def.tiles[1]..'^visionlib_circle_carving_overlay.png', def.tiles[1]..'^visionlib_pillar_overlay_2.png'}, paramtype2 = 'facedir',	on_place = minetest.rotate_node}))
	
	if recipes then
		minetest.register_craft({
			output=node..'_bricks 4',
			recipe={
				{node,node},
				{node,node}
			}
		...
		minetest.register_craft({
			output=node..'_tiles 4',
			recipe={
				{node..'_bricks',node..'_bricks'},
				{node..'_bricks',node..'_bricks'}
			}
		})
		...
		minetest.register_craft({
			type='shapeless',
			output=node,
			recipe={node..'_bevelled'}
		})
		
		minetest.register_craft({
			output=node..'_pillar 2',
			recipe={
				{node},
				{node}
			}
...
visionLib._sMaterials={
	...
	['insulator_ceramic']=function()
		visionLib.Material.Create('insulator_ceramic','Insulating Ceramic','stonelike','E7D4C6a0')
	end,
}
...
function visionLib.Material.Create(name, desc, ish, color, alloy)
	...
	elseif ish=='stonelike' then
		...
		minetest.register_craftitem(':vision_lib:'..name..'_chunk', {
			description = desc..' Chunk',
			inventory_image ='visionlib_chunk.png^[colorize:#'..color,
			groups={['nugget_'..name]=1, [name]=1, nugget=1},
		})

		minetest.register_node(':vision_lib:'..name..'_block', {
			description=desc..' Block',
			tiles={'visionlib_stoneyblock.png^[colorize:#'..color},
			is_ground_content=false,
			groups={cracky=2, stone=1, [name]=1, ['block_'..name]=1, stone_block=1},
			--sounds=visionLib.Sound.Stone(),
		})
		
		minetest.register_craft({
			type='shapeless',
			output='vision_lib:'..name..'_chunk 9',
			recipe={'vision_lib:'..name..'_block'}
		})
		...
		minetest.register_craft({
			output='vision_lib:'..name..'_block',
			recipe={
				{'vision_lib:'..name..'_chunk','vision_lib:'..name..'_chunk','vision_lib:'..name..'_chunk'},
				{'vision_lib:'..name..'_chunk','vision_lib:'..name..'_chunk','vision_lib:'..name..'_chunk'},
				{'vision_lib:'..name..'_chunk','vision_lib:'..name..'_chunk','vision_lib:'..name..'_chunk'}
			}
		})
...
if minetest.get_modpath('default') then
	...
	visionLib.Material.CreateVariants('default:dirt', true)
	visionLib.Material.CreateVariants('default:cobble', true)
	visionLib.Material.CreateVariants('default:coalblock', true)
	visionLib.Material.CreateVariants('default:clay', true)
	visionLib.Material.CreateVariants('default:copperblock', true)
	visionLib.Material.CreateVariants('default:coral_skeleton', true)
	visionLib.Material.CreateVariants('default:desert_cobble', true)
	visionLib.Material.CreateVariants('default:dry_dirt', true)
	visionLib.Material.CreateVariants('default:ice', true)
	visionLib.Material.CreateVariants('default:goldblock', true)
	visionLib.Material.CreateVariants('default:mossycobble', true)
	visionLib.Material.CreateVariants('default:permafrost', true)
	visionLib.Material.CreateVariants('default:snowblock', true)
	visionLib.Material.CreateVariants('default:steelblock', true)
	visionLib.Material.CreateVariants('default:tinblock', true)
end
